...
class _CommentState extends State<CommentView> {
            ...
            children: [
              ...
              Padding(
                ...
                child: JwtPayload.sub == null
                    ...
                    : CreateCommentView(
                        postId: widget.postId, callback: upComment),
              ),
              Padding(
  ...
  Future getComment({String subId = ''}) async {
    ...
    future.then((response) {
      _comments = response == null
          ...
          : response.data
              .map<Comment>((e) =>
                  Comment(subCommentAggreGate: SubCommentAggreGate.fromJson(e)))
              .toList();
      setState(() {});
    }).catchError((error) {
  ...
  Widget listSubCommentView(
      {required double paddingLeft, required List<Comment> comments}) {
    return Container(
  ...
  Widget subCommentView(
      {required Comment comment, required List<Comment> comments}) {
    return Container(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          headerComment(comment.subCommentAggreGate.user,
              comment.subCommentAggreGate.updatedAt),
          comment.isEdit
              ? Column(
            ...
            child: comment.isEdit
                ...
                : Row(
                    ...
                    children: [
                      ...
                      menuComment(
                          username: comment.subCommentAggreGate.username,
                          comment: comment,
                          comments: comments),
                    ],
                  ),
                        ...
                        callback: (SubCommentAggreGate subCom) {
                          setState(() {
                            comment.comments.insert(
                                0, Comment(subCommentAggreGate: subCom));
                            comment.isReply = false;
                          });
                        })
                  ],
          ...
          comment.comments.isEmpty
              ? Container()
              : listSubCommentView(paddingLeft: 24, comments: comment.comments),
          Container(
            padding: EdgeInsets.only(left: 32),
            child: (comment.subCommentAggreGate.right >
                        comment.subCommentAggreGate.left + 1 &&
                    !comment.isShowChildren)
                ? InkWell(
                    onTap: () {
                      Future<Response<dynamic>> future =
                          _commentRepository.getSubComment(
                              widget.postId, comment.subCommentAggreGate.id);
                      future.then((response) {
                        comment.comments = response == null
                            ? []
                            : response.data
                                .map<Comment>((e) => Comment(
                                    subCommentAggreGate:
                                        SubCommentAggreGate.fromJson(e)))
                                .toList();
  ...
  List<Widget> menuSignIn(List<Comment> comments, Comment comment) {
    return <Widget>[
      MenuItemButton(
          onPressed: () {
            setState(() {
              if (!comment.isEdit) comment.isEdit = true;
            });
          },
          child: Row(
            children: [
      ...
      MenuItemButton(
          onPressed: () {
            _showConfirmationDialog(
                context, comments, comment.subCommentAggreGate.id);
          },
          child: Row(
            children: [
              Icon(Icons.delete),
  ...
  _showConfirmationDialog(
      BuildContext context, List<Comment> comments, String subId) {
          ...
          actions: <Widget>[
            ...
            Container(
              ...
              child: FloatingActionButton(
                ...
                onPressed: () {
                  var future =
                      _commentRepository.deleteSubComment(widget.postId, subId);
                  future.then((response) {
                    bool result = response.data;
                    if (!result) return;
                    int index = comments.indexWhere(
                        (item) => item.subCommentAggreGate.id == subId);
                    if (index == -1) return;
                    setState(() {
  ...
  _showConfirmCancel(
      BuildContext context, Comment comment, Function() callback) {
    return showDialog<bool>(
      context: context,
      builder: (context) {
        return AlertDialog(
